#!/usr/bin/env bash
# execution provider — artifact.sh
#
# Backs `execution:<owner>/<repo>/<pr-number>` artifacts via `gh`. See
# ../SKILL.md for the full backend mapping.
#
...
parse_uri() {
  local u="$1"
  # execution:owner/repo/pr-number
  local rest="${u#execution:}"
  local repo="${rest%/*}"
  local pr="${rest##*/}"
...
case "$cmd" in
  get)
    ...
    body=$(gh pr view "$pr" --repo "$repo" --json body,state,assignees,title,url 2>/dev/null) \
      || die "not-found"
    jq --arg uri "$uri" '. + {uri:$uri, kind:"execution"}' <<< "$body"
    ;;
  ...
  create)
    ...
    body=$(printf '%s %s -->\n\n%s\n\n%s {"steps":[]} -->\n' \
      "$SUMMARY_TAG" "$(jq -c '{version:1, definition:.definition, execution_id:.execution_id, started_at:.started_at, status:"running"}' <<< "$data")" \
      "$summary" "$LEDGER_TAG")
    ...
    jq -n --arg uri "execution:$repo/$num" --arg url "$created" \
      '{uri:$uri, url:$url, kind:"execution", status:"running"}'
    ;;
  ...
  update)
    ...
    # Rewrite body preserving prose; only wf:summary + wf:ledger sections change.
    existing=$(gh pr view "$pr" --repo "$repo" --json body --jq .body)
    # This is a stub: production impl would parse the markers and rewrite in place.
  ...
  list)
    ...
    if [ -z "$repo" ]; then die "filter.repo required"; fi
    results=$(gh pr list --repo "$repo" --state "$state" --search "in:body wf:summary" \
      --json number,title,state,assignees,url 2>/dev/null || echo '[]')
    jq --arg repo "$repo" '{entries: [.[] | {uri:("execution:" + $repo + "/" + (.number|tostring)), title, state, assignees: (.assignees | map(.login)), url}]}' <<< "$results"
    ;;
