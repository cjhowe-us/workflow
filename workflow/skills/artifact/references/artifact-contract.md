...
- `<kind>` — the artifact kind this provider manages (`execution`, `gh-pr`, `document`, ...).
...
Query artifacts of this kind. `F` is a provider-specific filter string or a path to a filter JSON
doc. Output: `{"entries": [{...}, ...]}`.
...
other artifact kinds may map their backend state onto this enumeration (e.g. `gh-pr`: `open` →
