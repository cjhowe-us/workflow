...
| 2026-04-19 | **First-run seed + explicit storage bindings + shared auth.** The `default` workflow becomes a four-step DAG (`seed` → `tutor` → `interpret` → `act`); the `/workflow` skill injects `initialized` and `tutor_completed` flags derived from preferences so conditional steps skip cleanly on subsequent runs. `seed` asks one `AskUserQuestion` per backend slot — all three backends (execution state = `execution-gh-pr`, lock/presence = `gh-gist`, dependency overlay = `gh-issue`) are pluggable and optional; defaults are proposals the user confirms or swaps per slot. Presence gist becomes a first-class `presence` scheme backed by `presence-gh-gist`. GitHub auth is treated as a TTL'd artifact (`workflowlib.auth` wraps `artifactlib_gh.gh.auth_status`) cached under `$ARTIFACT_CONFIG_DIR/workflow/gh_auth.json`, shared across the three plugins so multi-worker dispatch doesn't re-shell `gh auth status` per call. Invariant #11 remains — durable state lives on GitHub by default — but the choice is now written explicitly to `preferences:workspace/<repo-hash>.extra.storage_state` rather than implied. |
